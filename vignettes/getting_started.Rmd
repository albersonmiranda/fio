...
`iom` is a R6 class that is the main data structure used by {fio} to store input-output data. The `iom` class has the following slots:
...
- `final_demand`: a matrix for final demand.
- `added_value`: a vector for added value.
...
- `added_value_final_demand`: a matrix with all others rows composing added value.
