...
Other methods available are `compute_leontief_inverse()`, `compute_multiplier_output()`, `compute_influence_field()`, `compute_key_sectors()`, `compute_allocation_coeff()`, `compute_hypothetical_extraction()`, `compute_ghosh_inverse()`, and `close_model()`. Their names are clear and self explanatory. You can also chain these methods to do all at once:
...
The `close_model()` method allows you to treat households and/or government as endogenous sectors by moving them from final demand to the intermediate transactions matrix. This is useful for analyzing the economy as a closed system.

```{r closed_model, eval=FALSE}
# Create an iom object with household consumption and wages
my_closed_iom <- fio::iom$new(
  id = "closed_example",
...
my_closed_iom$close_model("household")

# The intermediate transactions matrix now includes the household sector
print(my_closed_iom$intermediate_transactions)
```

When closing the model:
- Household consumption becomes a new column in the intermediate transactions matrix
- Wages become a new row in the intermediate transactions matrix  
- Both are removed from their respective final demand and value-added matrices
- Total production is updated to include the new endogenous sector

The same process applies when closing with the government sector using taxes and government consumption.
