...
The results show that `r interdependence$country[which.max(interdependence$self_reliance)]` has the highest self-reliance (`r round(max(interdependence$self_reliance), 4)`). Open economies such as `r interdependence$country[which.max(interdependence$total_spillover_out)]` show the largest total spillover out (`r round(max(interdependence$total_spillover_out), 2)`).

### USA spillover in vs out

In this 2000 MRIO, the USA often ranks low on **total spillover out** and high on **total spillover in**. That pattern is consistent with IO multiplier definitions, not necessarily with a "demand engine" narrative:

- **Spillover out** measures extra output required *abroad* when demand shocks hit *USA* sectors. The large USA domestic block in **L** absorbs most of the response.
- **Spillover in** measures extra output required *in the USA* when demand shocks hit *foreign* sectors—high when the USA is an upstream supplier in global value chains.

```{r usa_spillover, echo=FALSE}
usa_row <- interdependence[interdependence$country == "USA", ]
```

For the USA: spillover out = `r round(usa_row$total_spillover_out, 2)`, spillover in = `r round(usa_row$total_spillover_in, 2)`, balance = `r round(usa_row$spillover_balance, 2)`, export share = `r round(usa_row$spillover_export_share, 3)`.
...
For example, `net["USA", "CHN"] > 0` means a unit final-demand shock in China induces more output response in the USA than a comparable shock in the USA induces in China—not that the USA "exports" more spillover to the world in aggregate. The USA's negative **spillover balance** in `get_regional_interdependence()` confirms it is a net recipient in total cross-region spillovers for this dataset.
